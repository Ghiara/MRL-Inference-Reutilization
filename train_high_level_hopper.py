...
def rollout(env, encoder, decoder, simple_agent, high_level_controller, transfer_function, 
    ...
    for episode in range(30000):
        ...
        for path_length in range(max_path_len):
            ...
            # action_prev[[1,2,4]] = 0 * action_prev[[1,2,4]]
            action = torch.zeros(2).to(DEVICE)
            ...
            action[1] = action[1] * range_dict['velocity_x'][1]
            r = 0
            for i in range(sim_time_steps):
        ...
        if plot:
            ...
            for i, x_vel in enumerate(x_vel_plot):
                ...
                ax2.plot(np.arange(max_path_len), np.ones(max_path_len)*tasks_vel[i][1], linestyle='--', color=color)
...
if __name__ == "__main__":
    ...
    high_level_controller = SAC(n_states=14,
                n_actions=3,
