...
def rollout(env, encoder, decoder, optimizer, simple_agent, transfer_function, memory, 
    ...
    L2_pos, L2_vel = 0,0
    metadata_file_path = os.path.join(save_video_path, experiment_name, 'metadata.tsv')
    tensor_file_path = os.path.join(save_video_path, experiment_name, 'tensors.tsv')
    os.makedirs(os.path.dirname(metadata_file_path), exist_ok=True)
    os.makedirs(os.path.dirname(tensor_file_path), exist_ok=True)
    metadata_file = open(metadata_file_path, 'w')
    tensor_file = open(tensor_file_path, 'w')
    ...
    for episode in range(num_trajectories):
        ...
        for path_length in range(max_path_len):
            ...
            if task_prediction in [env.config.get('tasks',{}).get('goal_front'), env.config.get('tasks',{}).get('goal_back')]:
                ...
                if simple_action.item()>0:
                    simple_obs[env.config.get('tasks',{}).get('forward_vel')] = np.clip(_simple_obs[1].item(), -4,4)
                else:
                    simple_obs[env.config.get('tasks',{}).get('backward_vel')] = np.clip(_simple_obs[1].item(), -4,4)
...
if __name__ == "__main__":
    ...
    inference_path = '/home/ubuntu/juan/melts/output/toy1d-multi-task/2024_09_16_12_23_15_default_true_gmm'
    ...
    #     environment = HalfCheetahMixtureEnv,
    #     experiments_repo = '/home/ubuntu/juan/Meta-RL/experiments_transfer_function/',
    #     experiment_name = 'new_cheetah_training/half_cheetah_21_06',
    #     epoch = 1700,
    # )
    complex_agent_config = dict(
        environment = HalfCheetahMixtureEnv,
