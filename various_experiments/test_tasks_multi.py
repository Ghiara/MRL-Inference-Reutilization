...
experiment_name = 'cheetah_retrain_beta_0.1'
    ...
    for episode in range(len(tasks)):
        ...
        if plot:
            ...
            for i, rot in enumerate(rot_plot):
                ...
                # Plot position on the first (left) axis
                ax3.plot(np.arange(len(rot)), np.array(rot), label='Rot', color=color)
                # if tasks[i][0]!=0:
                ax3.plot(np.arange(len(rot)), np.ones(len(rot))*tasks_rot[i], linestyle='--', color=color, alpha=0.5)

            ax3.set_xlabel('Time (s)', fontsize=32)
            # ax3.set_xlabel('Time (s)')
            ax3.set_ylabel('Rot(m)', fontsize=32)
...
if __name__ == "__main__":
    ...
    inference_paths = [
        {'name': 'multi', 'path': '/home/ubuntu/juan/melts/output/toy1d-multi-task/2024_09_14_20_58_59_default_true_gmm'},
    ...
    for inference in inference_paths:
        ...
        tasks = [
            ...
            # {'base_task':'stand_back', 'specification':0.8},
            {'base_task':'stand_back', 'specification':0.5},
            {'base_task':'stand_back', 'specification':0.1},
            # {'base_task':'backward_vel', 'specification':1.0},
            # {'base_task':'forward_vel', 'specification':1.0},
            {'base_task':'stand_front', 'specification':0.1},
            {'base_task':'stand_front', 'specification':0.5},
            # {'base_task':'stand_front', 'specification':0.8},
                 ]
