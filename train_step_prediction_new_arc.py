...
experiment_name = 'train_step_predictor_beta0.01_bigger_normalize'
...
def log_all(agent, path, q1_loss, policy_loss, rew, episode, save_network='high_level'):
    '''
...
def rollout(env, encoder, decoder, high_level_controller, step_predictor, transfer_function_pos, transfer_function_vel, 
            variant, obs_dim, actions_dim, max_path_len, 
            n_tasks, inner_loop_steps, save_video_path, max_steps=20, beta = 0.01, batch_size=10, policy_update_steps=128):
    ...
    low_value_loss_history, low_q_loss_history, low_policy_loss_history, low_rew_history = [], [], [], []
    ...
    for episode in range(30000):
        ...
        for batch in tqdm(range(batch_size)):
            ...
            for path_length in range(max_path_len):
                ...
                if action[0] != 0:
                    action_normalize = action[0].item()
                    action[0] = action[0] + env.sim.data.qpos[0]
                    desired_state = torch.tensor([action[0], 0])
                    transfer_function = transfer_function_pos
                else:
                    action[3] = action[3] * range_dict['velocity_x'][1]
                    desired_state = torch.tensor([0, action[3]])
                ...
                for i in range(sim_time_steps):
                    ...
                    goal_obs = action[[0,3]]
                    non_zero_mask = (simple_obs != 0) & (goal_obs != 0)
                    # Compute the difference where neither element is ze`ro
                    
                    step_r -= 0.2 * torch.sum(torch.square(action_prev)).detach().cpu().numpy()
                    obs = next_obs
                    if plot:
                        image = env.render()
                        frames.append(image)
                ...
                if action[0] != 0:
                    low_level_r = - np.abs(action[0].detach().cpu().numpy()-simple_obs[0])/np.abs(action_normalize) - beta * sim_time_steps
                    low_level_r = np.clip(low_level_r, -5, 1)
                else:
                    low_level_r = - np.abs(action[3].detach().cpu().numpy()-simple_obs[1])/np.abs(action[3].item()) - beta * sim_time_steps
                    low_level_r = np.clip(low_level_r, -5, 1)
            ...
            if plot:
                ...
                for i, x_pos in enumerate(x_pos_plot):
                    ...
                    ax1.plot(np.arange(max_path_len), np.array(x_pos), label='Position', color=color)
                    # if tasks[i][0]!=0:
                    ax1.plot(np.arange(max_path_len), np.ones(max_path_len)*tasks_pos[i][0], linestyle='--', color=color)
                ...
                for i, x_vel in enumerate(x_vel_plot):
                    ...
                    ax2.plot(np.arange(max_path_len), np.array(x_vel), label='Velocity', color=color)
                    # if tasks[i][3]!=0:
                    ax2.plot(np.arange(max_path_len), np.ones(max_path_len)*tasks_vel[i][0], linestyle='--', color=color)
                if x_vel_plot:
        ...
        print('Task:', task[0], 'Base task:', env.base_task)
        ...
        if episode % save_after_episodes == 0 and episode!=0:
            ...
            log_all(step_predictor, path, low_q_loss_history, low_policy_loss_history, low_rew_history, episode, save_network='low_level')
...
if __name__ == "__main__":
    ...
    decoder = None
    step_predictor = SAC(n_states=20,
                n_actions=1,
