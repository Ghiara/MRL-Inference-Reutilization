...
experiment_name = 'train_low_and_high/'
...
def rollout(env, encoder, decoder, high_level_controller, transfer_function, 
    ...
    for episode in range(30000):
        ...
        for path_length in range(max_path_len):
            ...
            for i in range(sim_time_steps):
                complex_action = transfer_function.choose_action(obs, action.cpu().detach().numpy(), torch=True).squeeze()
                ...
                low_level_r = torch.abs(torch.where(non_zero_mask, torch.tensor(simple_obs).to('cuda') - goal_obs, 0))[0].item()
            ...
            low_level_losses = transfer_function.train(episode, False)
        ...
        if episode % save_after_episodes == 0 and episode!=0:
            ...
            log_all(transfer_function, path, low_q_loss_history, low_policy_loss_history, low_rew_history, episode, save_network='low_level')
        ...
        if plot:
            ...
            for i, x_vel in enumerate(x_vel_plot):
                ...
                # if tasks[i][3]!=0:
                ax2.plot(np.arange(max_path_len), np.ones(max_path_len)*tasks_vel[i][3], linestyle='--', color=color)
...
if __name__ == "__main__":
    ...
    inference_path = '/home/ubuntu/juan/melts/output/toy1d-multi-task/2024_04_11_09_25_22_default_true_gmm'
    ...
    transfer_function = SAC(n_states=20,
                n_actions=6,
                task_dim = 5,
                ...
                lr=3e-4,
                action_bounds=[-50,50],
                reward_scale=1, pretrained=pretrained_transfer)
    output_action_dim = 8
    ...
    rollout(env, encoder, decoder, high_level_controller,
