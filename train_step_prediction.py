...
experiment_name = 'train_step_predictor_redo_new_cheetah'
...
def rollout(env, encoder, decoder, high_level_controller, step_predictor, transfer_function, 
            ...
            n_tasks, inner_loop_steps, save_video_path, max_steps=sim_time_steps, beta = 0.01, batch_size=batch_size, policy_update_steps=policy_update_steps):
    ...
    for episode in range(30000):
        ...
        for batch in tqdm(range(batch_size)):
            ...
            sim_time_steps_list = []
            # if env.base_task in [env.config.get('tasks',{}).get('goal_front'), 
            #                             env.config.get('tasks',{}).get('goal_back')]:
            #     max_path_len = 600
            # else:
            #     max_path_len = 300
            ...
            while path_length < max_path_len:
                ...
                if base_task_pred == env.config.get('tasks',{}).get('goal_front'):
                    ...
                    desired_state = torch.tensor([action[base_task_pred], 0])
                    ...
                    desired_state = torch.tensor([0, action[base_task_pred]])
                ...
                for i in range(sim_time_steps):
                    ...
                    step_r -= 0.01 * torch.sum(torch.abs(penalty_r)).detach().cpu().numpy()
                    step_r = step_r.clip(-2, 0)
                    obs = next_obs
                    if plot:
                        image = env.render()
                        frames.append(image)
            ...
            if plot and len(tasks_pos)>1 and len(tasks_pos)>1:
                ...
                for i, x_pos in enumerate(x_pos_plot):
                    ...
                    # if tasks[i][0]!=0:
                    ax1.plot(np.arange(len(x_pos)), np.ones(len(x_pos))*tasks_pos[i], linestyle='--', c=color, alpha=0.5)
                ...
                for i, x_vel in enumerate(x_vel_plot):
                    ...
                    ax2.plot(np.arange(len(x_vel)), np.array(x_vel), label='Velocity', c=color)
                    # if tasks[i][3]!=0:
