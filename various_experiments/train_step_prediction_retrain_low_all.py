...
experiment_name = 'cheetah_retrain_low_all_29_08'
...
def rollout(env, encoder, decoder, high_level_controller, step_predictor, transfer_function, 
            ...
            n_tasks, inner_loop_steps, save_video_path, max_steps=sim_max_time_steps, beta = 0.01, batch_size=batch_size, policy_update_steps=policy_update_steps):
            ...
            while (not done) and path_length < max_path_len:
                ...
                while (i < sim_time_steps) and (not done):
                    ...
                    if action[env.config.get('tasks',{}).get('goal_front')] != 0:
                        sim_steps_r = - np.abs(action[env.config['tasks']['goal_front']].detach().cpu().numpy()-simple_obs[0])/np.abs(action_normalize)
                        sim_steps_r = np.clip(sim_steps_r, -5, 1)
                    elif action[env.config.get('tasks',{}).get('goal_back')] != 0:
                        sim_steps_r = - np.abs(action[env.config['tasks']['goal_back']].detach().cpu().numpy()-simple_obs[0])/np.abs(action_normalize)
                        sim_steps_r = np.clip(sim_steps_r, -5, 1)
                    elif action[env.config.get('tasks',{}).get('forward_vel')] != 0:
                        sim_steps_r = - np.abs(action[env.config['tasks']['forward_vel']].detach().cpu().numpy()-simple_obs[1])/np.abs(action[env.config['tasks']['forward_vel']].item())
                        sim_steps_r = np.clip(sim_steps_r, -5, 1)
                    elif action[env.config.get('tasks',{}).get('backward_vel')] != 0:
                        sim_steps_r = - np.abs(action[env.config['tasks']['backward_vel']].detach().cpu().numpy()-simple_obs[1])/np.abs(action[env.config['tasks']['backward_vel']].item())
                        sim_steps_r = np.clip(sim_steps_r, -5, 1)
                    elif base_task_pred in [env.config.get('tasks',{}).get('stand_front')]:
                        sim_steps_r = - np.abs(action[env.config['tasks']['stand_front']].detach().cpu().numpy()-env.sim.data.qpos[2])/np.abs(action[env.config['tasks']['stand_front']].item()) - beta * sim_time_steps
                        sim_steps_r = np.clip(sim_steps_r, -5, 1)
                    elif base_task_pred in [env.config.get('tasks',{}).get('stand_back')]:
                        sim_steps_r = - np.abs(action[env.config['tasks']['stand_back']].detach().cpu().numpy()-env.sim.data.qpos[2])/np.abs(action[env.config['tasks']['stand_back']].item()) - beta * sim_time_steps
                        sim_steps_r = np.clip(sim_steps_r, -5, 1)
                    elif base_task_pred in [env.config.get('tasks',{}).get('jump')]:
                        sim_steps_r = - np.abs(action[env.config['tasks']['jump']].detach().cpu().numpy()-np.abs(env.sim.data.qvel[1]))/np.abs(action[env.config['tasks']['jump']].item()) - beta * sim_time_steps
                        sim_steps_r = np.clip(sim_steps_r, -5, 1)
...
if __name__ == "__main__":
    ...
    inference_path = '/home/ubuntu/juan/melts/output/toy1d-multi-task/2024_09_02_07_29_57_default_true_gmm'
