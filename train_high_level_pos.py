...
experiment_name = 'cheetah_hierarchy_only_pos'
...
def rollout(env, encoder, decoder, simple_agent, high_level_controller, transfer_function_pos, 
            variant, obs_dim, actions_dim, max_path_len, 
            n_tasks, inner_loop_steps, save_video_path, reward_scale=5):
    ...
    for episode in range(30000):
        ...
        for path_length in range(max_path_len):
            ...
            action_prev = high_level_controller.choose_action(obs.squeeze(), mu.cpu().detach().numpy().squeeze(), torch=True).squeeze()
            ...
            for i in range(sim_time_steps):
                ...
                step_r = step_r_before - 0.2 * torch.sum(torch.square(action_prev)).detach().cpu().numpy()
                obs = next_obs
                if plot:
                    image = env.render()
                    frames.append(image)
        ...
        if episode % save_after_episodes == 0 and episode!=0:
            log_all(high_level_controller, path, q_loss_history, policy_loss_history, rew_history, episode)
...
if __name__ == "__main__":
    ...
    rollout(env, encoder, decoder, simple_agent, high_level_controller,
                                        transfer_function_pos, variant, obs_dim, action_dim, 
                                        max_path_len, n_tasks=1, inner_loop_steps=10, save_video_path=inference_path)
