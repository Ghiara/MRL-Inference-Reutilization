...
def main(path_to_folder, use_plots=None, plot_type=0, name=None, save_=True):
    ...
    for fig_nr, base_task in enumerate(plotting_data.keys()) if use_plots is None else enumerate(use_plots):
        ...
        axs[fig_nr].set_xticks((np.linspace(axs[fig_nr].get_xlim()[0] + 1, axs[fig_nr].get_xlim()[1] - 1, 5) * 10.).astype(int) / 10.)
        axs[fig_nr].set_yticks((np.linspace(0. if spec_min > 0. else spec_min, 0. if spec_max < 0. else spec_max, 5) * 10.).astype(int) / 10.)
